...
>[!TIP]
> 💡 **交互式学习资源**：你可以直接在浏览器中打开本目录下的 [排序算法可视化工具](file:///run/media/zhongpu/DATA/projects/github/data-structure-in-c-notes/chapter10/visualizer.html)。它支持随机生成/手动输入数组，选择各种经典算法并实时观察动画；特别是对于**堆排序**，它能同步展示二叉堆树形结构的节点下沉与重建过程。
...
基于上面的定义，任何基于交互相邻元素的排序算法（如插入排序）在平均情况下需要 $O(N^2)$ 的时间复杂度，因为它需要处理平均 $N(N-1)/4$ 个逆序对；注意，此时 $N^2$ 是上界，也是下界（low bound）。

另外一个简单的排序算法是冒泡排序（Bubble Sort），它也是基于交互相邻元素的排序算法，性能分析同样适用。
...
>[!NOTE]
> 不难发现，堆排序的结果是一个升序的数组；如果我们使用最小堆，那么结果就是一个降序的数组。
